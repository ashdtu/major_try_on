...
#tf.flags.DEFINE_string("pose_dir", "VITON_test/test_data/pose/",
#                       "Directory containing poses.")
tf.flags.DEFINE_string("segment_dir", "VITON_test/test_data/segment/",
                       "Directory containing human segmentations.")
tf.flags.DEFINE_string("image_dir", "VITON_test/test_data/tops/",
                       "Directory containing product and person images.")
tf.flags.DEFINE_string("test_label",
                       "VITON_test/test_data/test_labels.txt",
                       "File containing labels for testing.")
tf.flags.DEFINE_string("result_dir", "results/test_1/",
                       "Folder containing the results of testing.")

tf.flags.DEFINE_integer("begin", "0", "")
tf.flags.DEFINE_integer("end", "2032", "")
tf.logging.set_verbosity(tf.logging.INFO)
some=open('/home/ash/VITON/VITON_test/test_data/pose_mat.pkl','rb')
pose_dict=pkl.load(some)
...
def _process_image(image_name, product_image_name, sess,
  ...
  segment_raw=io.imread(FLAGS.segment_dir+image_id+'.png', as_grey=True)
  segment_raw = process_segment_map(segment_raw, image.shape[0], image.shape[1])
  pose_raw=extract_pose_keypoints(pose_dict['candidate.'+image_id],pose_dict['subset.'+image_id])
  print(pose_raw)
